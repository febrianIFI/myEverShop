...
function TableContent({ loading = false, noHeader = false, onSort, currentSort, emptyMessage, className }) {
    ...
    return (react.createElement(react.Fragment, null,
        ...
        react.createElement(Table, { className: className },
            !noHeader && (react.createElement(TableHeader, null,
                react.createElement(TableRow, null, columns
                    .filter((col) => !col.isRemoved)
                    .map((col) => (react.createElement(TableHead, { key: col.key, className: `${col.header.className} ${col.sortable ? 'cursor-pointer' : ''}`, onClick: () => col.sortable && handleSort(col.key), style: { width: col.width } },
            ...
            react.createElement(TableBody, null, tableData.length === 0 ? (react.createElement(TableRow, null,
                react.createElement(TableCell, { colSpan: columns.filter((col) => !col.isRemoved).length }, emptyMessage))) : (tableData.map((row, rowIndex) => (react.createElement(TableRow, { key: rowIndex }, columns
                .filter((col) => !col.isRemoved)
                .map((col) => (react.createElement(TableCell, { key: col.key, className: col.className, style: { width: col.width } }, col.render
...
const DefaultCartItemList = ({ items, showPriceIncludingTax = true, loading = false, onSort, currentSort, onRemoveItem }) => {
    const columns = [
        {
            ...
            render: (row) => {
                ...
                return (react.createElement("div", { className: "flex justify-start gap-4" },
                    react.createElement("div", { className: "shrink-0" }, row.thumbnail ? (react.createElement(Image, { src: row.thumbnail, alt: row.productName, width: 80, height: 80, className: "rounded-md" })) : (react.createElement(ProductNoThumbnail, { width: 80, height: 80 }))),
                    react.createElement("div", { className: "font-medium flex flex-col gap-1 items-start h-full min-w-0 flex-1" },
                        react.createElement("div", { className: "font-semibold break-words w-full" }, row.productName), (_c = row.variantOptions) === null || _c === void 0 ? void 0 :
                        _c.map((option) => (react.createElement("span", { key: option.optionId, className: "text-xs" },
                            react.createElement("span", null, option.attributeName),
                            ":",
                            ' ',
                            react.createElement("span", { className: "text-muted-foreground" }, option.optionText)))),
                        react.createElement("span", { className: "text-sm text-muted-foreground" },
                            ...
                            " x ",
                            row.qty),
                        react.createElement("a", { href: "#", className: "text-destructive text-sm", onClick: (e) => {
                                e.preventDefault();
                                onRemoveItem === null || onRemoveItem === void 0 ? void 0 : onRemoveItem(row.cartItemId);
                            } }, _('Remove')), (_d = row.errors) === null || _d === void 0 ? void 0 :
                        _d.map((error, index) => (react.createElement("span", { key: index, className: "text-xs text-destructive" }, error))))));
            }
        },
        {
            key: 'qty',
    ...
    return (react.createElement(react.Fragment, null,
        ...
        react.createElement(ExtendableTable, { name: "shoppingCartItems", columns: columns, initialData: rows, loading: loading, emptyMessage: _('Your cart is empty'), onSort: onSort, currentSort: currentSort, className: "cart__items__table border-none table-fixed border-spacing-y-2 border-separate w-full" }),
...
function ShoppingCartEmpty() {
    return (react.createElement("div", { className: "empty-shopping-cart w-full flex justify-center", style: { marginTop: '150px' } },
        react.createElement("div", null,
            ...
            react.createElement("div", { className: "flex justify-center mt-5" },
                react.createElement(Button_Button, { size: 'lg', onClick: () => (window.location.href = '/') },
                    react.createElement("span", { className: "flex space-x-2" },
                        react.createElement("span", { className: "self-center" }, _('CONTINUE SHOPPING')),
                        ' ',
                        react.createElement("svg", { className: "self-center", style: { width: '2rem', height: '2rem' }, xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor" },
        ...
        react.createElement("div", { className: "grid gap-10 grid-cols-1 md:grid-cols-4" },
            ...
            react.createElement("div", { className: "col-span-1 md:col-span-1" },
                ...
                react.createElement("div", { className: "shopping-cart-checkout-btn flex justify-between mt-5" },
                    react.createElement(Button_Button, { onClick: () => (window.location.href = checkoutUrl), title: _('CHECKOUT'), variant: "default", size: 'lg', className: 'w-full' }, _('CHECKOUT'))),
...
/******/ 	var __webpack_exports__ = __webpack_require__.O(undefined, [4286], () => (__webpack_require__(7325)))
