...
function CustomerInfo({ order: { orderNumber, customerFullName, customerEmail, paymentMethodName, noShippingRequired, shippingAddress, billingAddress } }) {
    return (react.createElement("div", { className: "checkout-success-customer-info" },
        ...
        react.createElement("div", { className: "customer-info mt-7 mb-5" },
            react.createElement("div", { className: "grid grid-cols-2 gap-7" },
                ...
                react.createElement("div", null,
                    ...
                    react.createElement("div", { className: "text-textSubdued" }, noShippingRequired ? (_('No shipping required')) : (react.createElement(AddressSummary, { address: shippingAddress })))),
                react.createElement("div", null,
                    react.createElement("div", { className: "mb-2" },
        ...
        react.createElement(Button_Button, { variant: 'default', size: 'lg', onClick: () => (window.location.href = '/'), title: _('CONTINUE SHOPPING') }, _('CONTINUE SHOPPING'))));
}
...
const OrderSummaryItems = ({ items }) => {
    ...
    return (react.createElement("ul", { className: "order__item__summary__list divide-y divide-border border-b border-border mb-3" }, items.map((item) => (react.createElement("li", { key: item.uuid, className: "flex items-start py-3" },
        react.createElement("div", { className: "relative mr-4 self-center" },
            item.thumbnail && (react.createElement(Image, { width: 100, height: 100, src: item.thumbnail, alt: item.productName, className: "w-16 h-16 object-cover rounded border p-2 box-border border-border" })),
            !item.thumbnail && (react.createElement(ProductNoThumbnail, { className: "w-16 h-16 rounded border border-border p-2 box-border" })),
            react.createElement("span", { className: "absolute -top-2 -right-2 bg-muted rounded-full w-6 h-6 flex items-center justify-center text-muted-foreground text-sm" }, item.qty)),
        react.createElement("div", { className: "flex-1 min-w-0 items-start align-top" },
            react.createElement("div", { className: "font-semibold text-sm mb-1", title: item.productName }, item.productName.length > 50
...
function Summary({ order }) {
    ...
    return (react.createElement("div", { className: "checkout__summary h-full hidden md:block" },
        ...
        react.createElement(OrderTotalSummary, { shippingCost: priceIncludingTax
                ? order.shippingFeeInclTax.text
                : order.shippingFeeExclTax.text, subTotal: priceIncludingTax ? order.subTotalInclTax.text : order.subTotal.text, total: order.grandTotal.text, shippingMethod: order.shippingMethodName, coupon: order.coupon || '', discountAmount: order.discountAmount.text, taxAmount: order.totalTaxAmount.text })));
}
...
/******/ 	var __webpack_exports__ = __webpack_require__.O(undefined, [4286], () => (__webpack_require__(31751)))
