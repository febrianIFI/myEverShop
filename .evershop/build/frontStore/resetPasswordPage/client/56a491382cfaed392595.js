...
var mail = __webpack_require__(43954);
    ...
    return (react.createElement("div", { className: "flex justify-center items-center" },
        react.createElement("div", { className: `reset__password__form ${className}` },
            react.createElement("div", { className: "reset__password__form__inner" },
                ...
                error && react.createElement("div", { className: "text-destructive mb-2" }, error),
                react.createElement(Form, { id: "resetPasswordForm", form: form, action: action, method: "POST", onSuccess: (response) => {
                    ...
                    react.createElement(EmailField, { prefixIcon: react.createElement(mail/* default */.A, { className: "h-5 w-5" }), name: "email", label: _('Email'), placeholder: _('Email'), required: true, validation: {
                            required: _('Email is required')
                        } }),
                    react.createElement("div", { className: "reset__password__form__submit__button flex border-t border-divider mt-2 pt-2" },
                        react.createElement(Button_Button, { type: "submit", variant: 'default', onClick: () => {
                                document.getElementById('resetPasswordForm').dispatchEvent(new Event('submit', { cancelable: true, bubbles: true }));
                            }, isLoading: loading }, _('Reset Password'))))))));
};
    ...
    return (react.createElement("div", { className: "flex justify-center items-center h-full" },
        react.createElement("div", { className: "reset__password__success flex justify-center items-center pt-10 md:pt-36" },
            react.createElement("div", { className: "reset__password__success__inner max-w-md px-4" },
                react.createElement("p", { className: "text-center text-success" }, children)))));
}
const UpdateForm = ({ token, action, loginUrl }) => {
    const [success, setSuccess] = react.useState(false);
    const [error, setError] = react.useState(null);
    const form = (0,index_esm/* useForm */.mN)();
    return success ? (react.createElement(Success, null,
        react.createElement("div", null, _('Your password has been updated successfully.')),
        react.createElement("div", { className: "flex items-center" },
            react.createElement(Button_Button, { title: _('Go to Login'), type: "button", onClick: () => {
                    window.location.href = loginUrl;
                } }, _('Go to Login'))))) : (react.createElement("div", { className: "flex justify-center items-center" },
        react.createElement("div", { className: "update-password-form flex justify-center items-center" },
            react.createElement("div", { className: "update-password-form-inner" },
                react.createElement("h2", { className: "text-center mb-5" }, _('Enter your new password')),
                error && react.createElement("div", { className: "text-critical mb-2" }, error),
                react.createElement(Form, { form: form, id: "updatePasswordForm", action: action, method: "POST", onSuccess: (response) => {
                        if (!response.error) {
                            setSuccess(true);
                        }
                        else {
                    ...
                    react.createElement(PasswordField, { name: "password", placeholder: _('Password'), required: true, validation: {
                            required: _('Password is required')
                        } }),
                    react.createElement(InputField, { name: "token", type: "hidden", defaultValue: token }),
                    react.createElement("div", { className: "form-submit-button flex border-t border-divider mt-2 pt-2" },
                        react.createElement(Button_Button, { title: _('UPDATE PASSWORD'), type: "submit", onClick: () => {
                                document.getElementById('updatePasswordForm').dispatchEvent(new Event('submit', { cancelable: true, bubbles: true }));
                            }, isLoading: form.formState.isSubmitting }, _('UPDATE PASSWORD'))))))));
};
function ResetForm({ action }) {
    ...
    return success ? (react.createElement(Success, null, _('We have sent you an email with a link to reset your password. Please check your inbox.'))) : (react.createElement(ResetPasswordForm, { title: _('Reset Your Password'), subtitle: _('Please enter your email to receive a reset link'), className: "w-120 max-w-max md:max-w-[80%] bg-white rounded-3xl p-6 shadow-lg border border-divider", action: action, onSuccess: () => {
            setSuccess(true);
        } }));
}
function ResetPasswordPage({ requestAction, updateAction, loginUrl }) {
    const [token, setToken] = react.useState('');
    react.useEffect(() => {
        const urlParams = new URLSearchParams(window.location.search);
        const tokenParam = urlParams.get('token');
    ...
    return token ? (react.createElement(UpdateForm, { token: token, action: updateAction, loginUrl: loginUrl })) : (react.createElement(ResetForm, { action: requestAction }));
}
...
/******/ 	var __webpack_exports__ = __webpack_require__.O(undefined, [4286], () => (__webpack_require__(26866)))
